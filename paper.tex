...
insight into the complexity of transcriptomes~\cite{nagalakshmi2008transcriptional,mortazavi2008mapping,pan2008deep,Wang:2008ea}.
...
approximately 5\% of its true gene content~\cite{hillier2004sequence}.
Interestingly, over a hundred regions were present {\em only} in the local
assemblies, suggesting that the local assemblies might be recovering additional
exons.  Significant numbers of unique regions from both global and local
assemblies showed homology to the mouse genome, indicating that at least some
of these unique sequences represented real sequence content.
...
originated from sample processing and reverse
transcription~\cite{martin2011next}.  Thus the merged gene models produced by
Gimme represent the significant majority of the assemblable data.
...
approaches as well as expression data~\cite{flicek2013ensembl}.
...
% @CTB for discussion: what's surprising about all of this? so many splice
% variants, etc?  Note we need to go over pickrell paper carefully.  @LP I
% discuss about it below. Hope it helps.
