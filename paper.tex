...
\section*{Abstract}
Comparison of spliced reads and reference gene annotations has been
successfully used to discover alternative splicing in model
organisms. However, the method cannot be applied in organisms without
high-quality reference genome and gene annotations. In this article,
we introduce a pipeline, based on \emph{de novo} assembly, that
constructs gene models with splice variants.  The pipeline uses a
technique called local assembly that enhances the sensitivity of
alternative splicing detection.  We demonstrate that the pipeline
detects many novel splice variants in RNA-Seq data from chicken
spleen.  Many of these splice variants; however, are only detected by our
pipeline and not Cufflinks.  This indicates that the pipeline can be
used to facilitate splice variant detection from RNA-Seq.
...
Using a similar approach, Pickrell \emph{et al} identified more than
...
tissue-specific expression of isoforms\cite{Pickrell:2010gt}.  A
majority of unannotated isoforms found in both studies are from
alternative splicing of annotated exons. Only a small fraction of
isoforms contain previously unknown exons.
...
myoblast cell lines.  Guttman \emph{et al} used Scripture, a tool
...
can be overcome by \emph{de novo} assembly of short reads.

A number of \emph{de novo} assemblers have been used to reconstruct
...
Cufflinks or Scripture.  This suggests that a \emph{de novo} assembly
...
has been shown to be superior than other \emph{de novo} assemblers at
...
from \emph{de novo} assembly and mapping-based approach can be merged
...
(Table~\ref{single-end_map}).
% @CTB compared to what?
More importantly, we also mapped
...
publicly available ESTs or mRNAs are from other tissues.  Note that
this cross-validation suggests that the 18,573 novel splice junctions
{\em not} seen in publicly available ESTs and mRNAs are also likely to
be real splice junctions from spleen.
