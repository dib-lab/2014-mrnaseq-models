...
analysis.  However, we find that Ensembl models in chicken are missing a
substantial number of splice variants found in ESTs and mRNAs, and Cufflinks does
not recover a number of them.  A previous study by Schulz \emph{et
...
We find that Cufflinks almost exclusively finds splice junctions in
mouse Ensembl gene models, whereas our gene models include splice junctions that are not in
Ensembl gene models but that {\em are} well supported by ESTs (Figure~\ref{mus_venn}). 
...
We therefore recommend that a combination of Cufflinks models and
transcripts from {\em de novo assembly} be used to build gene models that
...
Both Cufflinks and our pipeline rely on a reference genome; therefore,
...
Archive.  Chicken RNA-Seq datasets were obtained from
sequencing of mRNAs from spleen of chicken line 6 and 7.
...
both datasets using khmer\cite{khmer} (@CTB what script? I may have just removed it :).  Then, 20-mers from a query dataset are
...
detects the similar amount of non-overlapped annotated junctions.}
\label{mus_venn}
\end{figure}
